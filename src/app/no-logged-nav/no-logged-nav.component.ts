...
export class NoLoggedNavComponent implements OnInit, OnDestroy {
  ...
  navChange(link: string) {
    ...
    const curr = this.newsService.links.indexOf(this.newsService.activeLink && this.newsService.links.indexOf(this.newsService.activeLink) !== -1 ? this.newsService.activeLink : this.newsService.links[0]);
  ...
  navClick(link: string) {
    if (link !== this.activeLink||this.router.url !== '/home') {
