...
export class CamerasComponent implements OnInit, OnDestroy, AfterViewInit {
    private manyOffers = false;
    fakestream: MediaStream;
    ...
    private miSubject = new BehaviorSubject<string[]>([]);
    ...
    ngOnInit() {
        ...
        this.caSubject.asObservable().pipe(takeUntil(this.onDestroy), map(async (cameraEvent) => {
            if (cameraEvent.peer) {
                console.log('peer --> ' + cameraEvent.peer + ' stream --> ' + this.shourenego.get(cameraEvent.peer));
                this.yostream = true;
                this.callDeep(cameraEvent.peer, cameraEvent.fakeStream);
                await this.handleNegotiationNeededEvent(this.offerOptions, cameraEvent.peer, cameraEvent.fakeStream.id, false)
                    .then(async () => {
                        if (cameraEvent.realStream) {
                           await this.setReplacement(cameraEvent.peer, cameraEvent.realStream.getVideoTracks()[0], cameraEvent.realStream);
                            ...
                            // if (cameraEvent.user) {
                            //     this.yostream = false;
                            //     await this.callMeUser(cameraEvent.user);
                            // }
                        }
                    });
            }
        })).subscribe(value1 => console.log(value1));
        this.miSubject.asObservable().pipe(takeUntil(this.onDestroy), map(async value => {
            if (value.length > 0) {
                await this.callRec(value, value.length - 1).then(async () => {
                    for (const key of this.shourenego.keys()) {
                        if (this.shourenego.get(key)) {
                            this.shourenego.set(key, false);
                        }
                    }
                    this.manyOffers = false;
                });
            }
    ...
    callRec = async (list, len) => {
        this.yostream = true;
        const faked = new MediaStream([this.localStream.getVideoTracks()[0]]);
        faked.removeTrack(faked.getVideoTracks()[0]);
        this.callDeep(list[len], faked);
        if (!this.peerLStream.get(list[len])) {
            this.peerLStream.set(list[len], [faked.id]);
        } else {
            this.peerLStream.get(list[len]).push(faked.id);
        }
        this.fakelist.get(list[len]).push(faked.id);
        if (len === 0) {
            return await this.handleNegotiationNeededEvent(this.offerOptions, list[len], faked.id, false);
        } else { await this.timer(3000); }
        await this.handleNegotiationNeededEvent(this.offerOptions, list[len], faked.id, false)
            .then(async value => await this.callRec(list, len - 1));
    }
    startCamera() {
    ...
    attachVideo(stream) {
        ...
        this.mediaRecorder = new MediaRecorder(stream, this.getMediaRecorderOptions());
        ...
        this.mediaRecorder.ondataavailable = e => {
            if (e.data) { this.chunks.push(e.data); }
        };
    }
                    ...
                    if ((!this.peerLStream.get(peer) || !this.peerLStream.get(peer).includes(stream.id)) &&
                        ...
                        try {
                            if (!isOther) {
                                ...
                                const faked = new MediaStream([this.localStream.getVideoTracks()[0]]);
                                faked.removeTrack(faked.getVideoTracks()[0]);
                                this.zone.run(() => this.caSubject.next({
                                    peer: peer,
                                    fakeStream: faked,
                                    realStream: stream
                                }));
       ...
       // this.yostream = false;
        ...
        for (const key of this.shourenego.keys()) {
            if (this.shourenego.get(key)) {
                ...
                const faked = new MediaStream([this.localStream.getVideoTracks()[0]]);
                faked.removeTrack(faked.getVideoTracks()[0]);
                this.zone.run(() => setTimeout(() =>  this.caSubject.next({peer: key, fakeStream: faked})));
                break;
            }
        }
        // if (hrt.length > 0) {
        ...
        //         this.zone.run(() => this.caSubject.next(hrt[i]));
        //         if (i !== hrt.length - 1) { await this.timer(3000); }
        //     }
    ...
    handleNegotiationNeededEvent = async (event, mypeer, isnew, streamf: boolean) => {
        try {
            ...
            if (this.peerList.get(mypeer).signalingState !== 'stable' || (streamf && mypeer === this.targetUsername)) {
            ...
            this.signalingConnection.sendToServer({
                ...
                target: mypeer,
                streams: mystreams,
                fake: isnew !== '' ? isnew : this.fakestream.id,
                polite: isnew !== '',
                event: 'offer',
            ...
            if ([...this.shourenego.values()].some(value1 => value1) && !this.manyOffers) {
                console.log('offer creation finaled', mypeer);
                this.manyOffers = true;
                setTimeout(() => this.callMeUser(mypeer).then(value => value));
            }
            // const hrt = [];
            // for (const key of this.shourenego.keys()) {
            //     if (this.shourenego.get(key)) {
            ...
            //     }
            // }
            // if (hrt.length > 0 && !this.manyOffers) {
            //     this.manyOffers = true;
            //     this.zone.run(() => this.miSubject.next(hrt));
            // }
        }
    }
    ...
    connectionAnswer = async (msg) => {
        try {
            ...
            if (msg.name === this.targetUsername) {
                ...
                const malis = [];
                if (this.peerLStream.get(msg.name)) { this.peerLStream.get(msg.name).forEach(h => malis.push(h)); }
                if (this.peerRStream.get(this.targetUsername)) {
                    this.peerRStream.get(this.targetUsername).forEach(hd => {
                        if (!malis.includes(hd)) {
                            malis.push(hd);
            ...
            for (const key of this.shourenego.keys()) {
                if (this.shourenego.get(key)) {
                    hrt.push(key);
                    console.log('caller to feed --> ' + key
                        + ' offerer --> ' + msg.target + ' answerer --> ' + msg.name);
                }
            }
            if (hrt.length > 0) {
                this.manyOffers = true;
                // await this.callMeUser(msg.name);
                this.zone.run(() => this.miSubject.next(hrt));
            } else {
                await this.checkThyPeers(this.targetUsername, false);
            }
        }
    }
    checkThyPeers = async (target, isOther) => {
        if (target === this.targetUsername && this.checkMyPeers(target)) {
            this.yostream = true;
            await this.setRemotePeerStreams(isOther);
        }
    }
    checkMyPeers = (target) => {
        if (this.peerList.size > 1) {
            return [...this.peerList.keys()].filter((peer) => peer !== target)
                .some((mypeer) => {
                    return !this.peerLStream.get(mypeer) || !this.peerRStream.get(target) || !this.peerRStream.get(target)
