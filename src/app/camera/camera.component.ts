...
export class CameraComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    attachVideo(stream) {
        ...
        this.renderer.setProperty(this.localVideo.nativeElement, 'srcObject', this.merger.result);
    ...
    setLocalStreams = async (isAnswerer) => {
      ...
      //  }
        for (const track of this.merger.result.getTracks()) {
            ...
            } else { this.peerList.get(this.targetUsername).addTransceiver(track, { streams: [this.merger.result] }); }
        }
    }
    gotRemoteStream = (e) => {
        this.trace('set RemoteStream by: ' + this.username + '--id: ' + e.streams[0].id + ' -clones length --> ' + this.clones.length);
